...
dengue_cases <-  dplyr::inner_join(postaldistricts, dengue_data_filtered, by = 'Postcode')


# Convert polygon to point using point on surface
dengue_cases <- st_centroid(dengue_cases)
...
leaflet() %>%
  ...
  addMarkers(data = dengue_cases, icon = custom_icons) %>%
