...
municipality <- st_read("data/arrondissement_4326.shp") 

# Ensure both datasets have the same CRS
crs_municipality <- st_crs(postaldistricts)

# Assuming 'municipality' is your sf object with polygons
municipality <- st_zm(municipality)

# Calculate centroids of the polygons
post_code_centroids <- st_centroid(postaldistricts)
...
duplicate_rows <- joined_data  %>%
  group_by(Postcode) %>%  # Group by the column of interest
...
valid_postcodes <-  unique(belgium_shp$Postcode)
...
write.csv(filtered_DENV_Belgium, 'data/clean_dengue_data.csv', row.names = FALSE)

```

  # Ensure the data is in the correct CRS (Coordinate Reference System) for Leaflet
  aedes_dengue_matches <- reactive({st_transform(aedes_dengue_matches, crs = 4326)})
  # WGS 84
  
  # Ensure the data is in the correct CRS (Coordinate Reference System) for Leaflet
  aedes_dengue_non_matches <- reactive({st_transform(a  aedes_dengue_non_matches, crs = 4326)})
  # WGS 84
