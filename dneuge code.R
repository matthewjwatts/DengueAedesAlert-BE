...
server <- function(input, output, session) {
  ...
  observeEvent(input$file1, {
    ...
    req(input$dataset_select)  # Ensure a dataset is selected
    ...
    if (dataset_type == "aedes") {
      uploaded_file <- read.csv(input$file1$datapath, header = TRUE, colClasses = c("Observation_ID" = "character"))
      
      # Validate columns for aedes_data
      ...
      if (!all(required_columns %in% colnames(uploaded_file))) {
        output$message <- renderText("Error: The uploaded file does not have the required columns for Aedes Data.")
        return(NULL)
      }
      
      # Validate data for aedes_data
